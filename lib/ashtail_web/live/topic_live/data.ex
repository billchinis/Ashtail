defmodule AshtailWeb.TopicLive.Data do
  @moduledoc """
  ...
  `Kafka.read_topic/2`, reusing the per-partition browser's message row
  components.

  Page mode covers paging and value expansion. The key/value/header/partition
  filter reads through the same function in scan mode: any active filter runs
  `Kafka.read_topic/2` in a `start_async` task instead of synchronously, so an
  uncapped scan never blocks the page. The time range works the same way: `from`
  and `to` are query params like every other filter field, entered directly (no
  preset control). The tail extends the per-partition browser's tail across
  every scoped partition: a LiveView-owned timer reads forward from `tail_from`
  (a per-partition offset map) through the same `Kafka.read_topic/2`, so the
  active filter applies to tailed rows exactly as it does to a page or a scan.
  New rows are prepended and the stream is trimmed to `page_size`. Turning
  tailing on returns to page 1, and the tail starts exactly where that page
  ended, once it has loaded.

  A scan is bounded: every scan reads at most `scan_budget/0` messages per read
  (`max_scanned`), and a read that stops at that limit short of a full page
  offers "Scan more", which continues from the returned cursor into the same
  page, keeping the rows and the count found so far (`scan_page` accumulates the
  completed reads). A running scan offers "Stop", which cancels the task and
  keeps what it found (`scan_state: :stopped`).
  """

  use AshtailWeb, :live_view

  alias Ashtail.Kafka
  alias Ashtail.Kafka.{BrokerError, Filter}
  ...
  # A non-binary `row` (a crafted payload) must not crash `Integer.parse/1` in
  # `remove_json_row/2`, the same as the offset handlers above: it simply cannot
  # match any row, so it is a no-op.
  ...
  # Stop: cancels the running scan and keeps what it found. A fresh
  ...
  # so it is switched off rather than left "live".
  ...
  # Scan more: continues a scan that stopped at the limit, into the same page:
  # the same filter, the remaining rows of the page, from the cursor the last
  # read returned. The URL is untouched, so a reload starts the page over from
  # its own cursor.
  def handle_event(
        "scan_more",
        _params,
  ...
  # `scan_id`, or the scan was cancelled): drop it.
  ...
  def handle_async({:scan, scan_id}, {:exit, reason}, %{assigns: %{scan_id: scan_id}}) do
    ...
    # a %BrokerError{} would hide a real bug.
  ...
  defp load(socket, topic, page_size, cursor, filter_params, scan_id) do
    case Kafka.parse_filter(filter_params) do
      {:error, errors} ->
        # A rejected filter must not tail at all: the form still shows the
        # rejected filter, so tailing on with no filter would silently show
        # every message. `older`/`newer` are reset too, since no read happened
        # and the previous filter's cursors no longer apply.
        socket
        |> assign(
          filter: Filter.none(),
  ...
  defp start_scan(socket, topic, filter, page_size, cursor, scan_id) do
    ...
    # `page_high` is reset so a tail switched on while this scan is still
    # running cannot arm itself from a stale, previous filter's high-water
    # mark — `toggle_tail/2` only trusts `page_high` once it reflects this
    # read.
    |> assign(
      scan_state: :running,
      scanned: 0,
  ...
  # One read of the scan, bounded by `scan_budget/0`. Shared by a fresh scan and
  # a "Scan more" continuation, which differ only in what they reset first.
  ...
  # what any seed topic holds.
  defp scan_budget, do: Application.get_env(:ashtail, :data_scan_budget, @scan_budget)

  # Test-only: a small chunk makes a scan slow enough to still be running
  # when Stop is clicked. Never set outside a test.
  defp scan_chunk_opts do
    case Application.get_env(:ashtail, :data_scan_chunk) do
      nil -> []
  ...
  # The cursor "Scan more" continues from, or `nil` when there is nothing to
  # continue: the page is full, the range is exhausted in the read direction, a
  # pattern halted the scan, or no filter is active (an unfiltered page never
  # stops short of a full page or the range's end).
  ...
  # Two-fetch rule: a successful page read must not clear an error the topic
  # summary fetch already set (broker_error keeps the first error of the two).
  ...
  defp apply_page(socket, page) do
    ...
    # link in the read direction waits until it is.
  ...
  # Arms the tail the moment a page-1 read completes while `tailing?` is true
  # and `tail_from` is still `nil`: that is either the read that just completed
  # after tailing was switched on, or the page-1 reload `toggle_tail/2` forced
  # when tailing was switched on from a cursor page. Once armed, later page-1
  # reads (a new filter, a new scan) leave `tail_from` alone.
  ...
  # Paging away from page 1 ends the tail: a non-`nil` cursor in the freshly
  # parsed URL stops it before the new page loads.
  ...
  # Turning tailing on returns to page 1. On page 1 with a completed read, the
  # tail is gap-free from the start: `tail_from` is set to that read's `high`
  # map straight away. From a cursor page, or while page 1's own scan is still
  # running, `tail_from` stays `nil` and `maybe_arm_tail/2` sets it once a
  # page-1 read completes.
  defp toggle_tail(socket, false), do: stop_tail(socket)

  # A currently rejected filter must never tail: with no valid filter, `load/5`
  # never runs a read, so `tail_from` could never be armed and the indicator
  # would be stuck showing "live" forever. Refusing the toggle outright, instead
  # of falling through to the catch-all clause below, keeps `tailing?` false and
  # the indicator correctly off.
  ...
  # `start_scan/6` clears `page_high` before a scan runs, but the explicit guard
  # also protects against a future read path that leaves a stale `page_high` in
  # place while `scan_state` is `:running`. Without it, switching the tail on
  # mid-scan could arm `tail_from` from a previous filter's completed page-1
  # read instead of waiting for the running scan's own result.
  defp toggle_tail(
  ...
  # Configurable, not just `@tail_interval_ms`, so a test can set it far longer
  # than its own run time: otherwise a real, correctly-timed tick firing
  # mid-test can mask a bug in what a tick was armed from, by quietly catching a
  # stale value up to the right one before the test gets to look at it.
  defp tail_interval_ms,
    do: Application.get_env(:ashtail, :data_tail_interval_ms, @tail_interval_ms)
  ...
  # running, reschedule and do nothing.
  ...
  # `:message_index` is trimmed to the same bounded window so the expander
  # keeps working for exactly what is on screen.
  ...
  # bounded `:message_index` map keyed by `{partition, offset}`.
  ...
  # `handle_params/3` is the only source of applied JSON rows: a row's id always
  # equals its position, since `DataParams.parse/1` already dropped blank-path
  # rows and renumbered the rest. `next_json_id` continues from there, so ids
  # are never reused across an apply.
  ...
  # Copies typed `path`/`op`/`value` into the matching row by id, so a re-render
  # (an added row, scan progress) never reverts what a person is typing. An id
  # the form does not know, or a non-binary field, is ignored.
  ...
  # unapplied form. Every JSON row surviving `DataParams`' normaliser already
  # has a non-blank path, so it always counts as one filter.
